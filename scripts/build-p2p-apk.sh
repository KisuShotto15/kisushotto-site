#!/usr/bin/env bash
# Builds a signed TWA APK for p2p.kisushotto.com
# Installs all prerequisites automatically on first run (~10-15 min)
set -e

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
KEYSTORE="$REPO_ROOT/p2p-twa.keystore"
KEY_ALIAS="p2p-key"
KEY_PASS="p2p2024"
...
android {
    namespace 'com.kisushotto.p2p'
    compileSdk 34
    defaultConfig {
        applicationId 'com.kisushotto.p2p'
        minSdk 21
        targetSdk 34
        versionCode 1
        versionName '1.0.0'
    ...
    signingConfigs {
        release {
            storeFile file('../../p2p-twa.keystore')
    ...
    <application
        ...
        <activity
            ...
            <meta-data
                ...
                android:value="https://p2p.kisushotto.com/" />
            <meta-data
            ...
            <intent-filter android:autoVerify="true">
                ...
                <data android:scheme="https" android:host="p2p.kisushotto.com" />
            </intent-filter>
        </activity>
        <service
...
const src = 'public/images/p2p-icon.svg';
...
Promise.all([
  makeIcon('p2p-twa/app/src/main/res/mipmap-mdpi/ic_launcher.png', 48),
  makeIcon('p2p-twa/app/src/main/res/mipmap-hdpi/ic_launcher.png', 72),
  makeIcon('p2p-twa/app/src/main/res/mipmap-xhdpi/ic_launcher.png', 96),
  makeIcon('p2p-twa/app/src/main/res/mipmap-xxhdpi/ic_launcher.png', 144),
  makeIcon('p2p-twa/app/src/main/res/mipmap-xxxhdpi/ic_launcher.png', 192),
  makeForeground('p2p-twa/app/src/main/res/drawable/ic_launcher_foreground.png', 432),
...
cp "$APK" "$REPO_ROOT/KsP2pMonitor.apk"
echo "[build] APK ready: $REPO_ROOT/KsP2pMonitor.apk  ($(du -sh "$REPO_ROOT/KsP2pMonitor.apk" | cut -f1))"
...
cat > "$REPO_ROOT/public/.well-known/p2p-assetlinks.json" << ALEOF
[{
  "relation": ["delegate_permission/common.handle_all_urls"],
  "target": {
    "namespace": "android_app",
    "package_name": "com.kisushotto.p2p",
    "sha256_cert_fingerprints": ["$FINGERPRINT"]
  }
}]
ALEOF
echo "[assetlinks] Written: public/.well-known/p2p-assetlinks.json"

# ── 9. Rebuild p2p-pwa with assetlinks ────────────────────────────────────────
echo "[pwa] Rebuilding p2p-pwa with assetlinks..."
cd "$REPO_ROOT"
bash scripts/build-p2p-pwa.sh

echo ""
echo "=============================="
echo " TWA APK build complete!"
echo "=============================="
echo " APK:  $REPO_ROOT/KsP2pMonitor.apk"
echo " SHA:  $FINGERPRINT"
echo ""
echo "Next steps:"
echo "  1. git add p2p-twa/ p2p-pwa/ public/.well-known/ .gitignore scripts/ KsP2pMonitor.apk"
echo "  2. git commit && git push   <- deploys assetlinks.json to p2p.kisushotto.com"
echo "  3. Transfer KsP2pMonitor.apk to phone and install"
echo "  4. Or: adb install \"$REPO_ROOT/KsP2pMonitor.apk\""
echo ""
