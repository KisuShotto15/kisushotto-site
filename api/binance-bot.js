...
export default async function handler(req, res) {
  ...
  if (path === '/bot-enable' || path === '/bot-disable' || path === '/bot-state' || path === '/bot-config') {
    try {
      ...
      const rows = await sql`SELECT enabled, config, ad_number, current_price, ad_amount, ad_hidden, ad_min_limit, last_reprice, last_tick, status, log FROM bot_state WHERE user_id = ${user.uid}`;
