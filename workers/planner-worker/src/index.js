...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET') {
      ...
      if (url.searchParams.has('backups')) {
        const list = await env.PLANNER_KV.list({ prefix: BAK_PREFIX });
      ...
      if (day) {
        const raw = await env.PLANNER_KV.get(BAK_PREFIX + day);
    ...
    if (request.method === 'POST') {
      ...
      if (day) {
        const raw = await env.PLANNER_KV.get(BAK_PREFIX + day);
        if (!raw) return json({ error: 'no hay respaldo de ' + day }, 404);
        await backup(env.PLANNER_KV);
        await env.PLANNER_KV.put(KV_KEY, raw);
      ...
      await env.PLANNER_KV.put(KV_KEY, body);
      return json({ ok: true });
    }
  ...
  async scheduled(_event, env, ctx) {
    ctx.waitUntil(backup(env.PLANNER_KV));
  },
};
