// nutrition-data-worker — estado de Nutricion en una clave de KV.
//
...
const BAK_PREFIX = 'nutrition-state-bak-';
const BAK_DAYS = 7;
// Tope de tamano: el estado real son decenas de KB (microCache es lo que mas pesa).
// Un cuerpo enorme solo puede ser un error o un abuso. Se mide en caracteres.
const MAX_CHARS = 2 * 1024 * 1024;
// Claves que el cliente siempre manda (ver `let S = ...` en nutrition/main.js).
// Si faltan, lo que llego no es el estado de Nutricion.
const REQUIRED = ['profiles', 'lastModified'];
...
function invalidState(body) {
  ...
  // Sin perfiles no hay nada que guardar: seria borrar las comidas de todos ellos.
  if (!data.profiles || typeof data.profiles !== 'object') return 'profiles debe ser un objeto';
  if (!Object.keys(data.profiles).length) return 'profiles vacio';
  return null;
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET') {
      ...
      if (url.searchParams.has('backups')) {
        const list = await env.NUTRITION_KV.list({ prefix: BAK_PREFIX });
      ...
      if (day) {
        const raw = await env.NUTRITION_KV.get(BAK_PREFIX + day);
    ...
    if (request.method === 'POST') {
      ...
      if (day) {
        const raw = await env.NUTRITION_KV.get(BAK_PREFIX + day);
        if (!raw) return json({ error: 'no hay respaldo de ' + day }, 404);
        await backup(env.NUTRITION_KV);
        await env.NUTRITION_KV.put(KV_KEY, raw);
  ...
  async scheduled(_event, env, ctx) {
    ctx.waitUntil(backup(env.NUTRITION_KV));
  },
};
